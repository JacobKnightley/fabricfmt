...
export function shouldSkipSpace(
  ...
  const prevWasHexBinaryPrefix =
    (context.prevTokenText.toUpperCase() === 'X' ||
  ...
  const prevWasUnaryAndCurrentIsUnary =
    context.prevTokenWasUnaryOperator && context.currentTokenIsUnaryOperator;
  ...
  return (
    ...
    (text === '(' &&
      (context.prevWasFunctionName || context.prevWasBuiltInFunctionKeyword)) ||
