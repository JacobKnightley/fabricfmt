/**
 * Tests for notebook-formatter.ts
 * Tests extraction and formatting of Spark SQL cells from Fabric notebook files
 ...
 * Tests use content-based detection since they don't have actual files.
 */
function inferExtension(content: string): string {
  const firstLine = content.split(/\r?\n/)[0]?.trim() ?? '';
  if (firstLine.startsWith('// Fabric notebook source')) return '.scala';
  if (firstLine.startsWith('-- Fabric notebook source')) return '.sql';
  if (firstLine.startsWith('# Fabric notebook source')) return '.py';
  return '.py';
}
...
/**
 * Format a Fabric notebook by parsing and reformatting Spark SQL cells.
 */
...
export function runMagicSqlSuite(): { passed: number; failed: number; results: Array<{ name: string; passed: boolean; input?: string; expected?: string; got?: string }> } {
  ...
  for (const tc of allTests) {
    const result = formatNotebookContent(tc.input);
