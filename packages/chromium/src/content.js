 ...
 * Uses @jacobknightley/fabricfmt for all formatting
 */

import { formatCell, initializePythonFormatter } from '@jacobknightley/fabricfmt';
 ...
 *   window.__fabricfmt_debug__ = true   // Enable debug logging
 *   window.__fabricfmt_debug__ = false  // Disable debug logging
 */
if (typeof window !== 'undefined' && window.__fabricfmt_debug__ === undefined) {
  window.__fabricfmt_debug__ = false;
}

const log = {
  /** Always shown - important state changes and results */
  info: (...args) => console.log('[fabricfmt]', ...args),
  
  /** Always shown - warnings */
  warn: (...args) => console.warn('[fabricfmt]', ...args),
  
  /** Always shown - errors */
  error: (...args) => console.error('[fabricfmt]', ...args),
  
  /** Only shown when debug enabled - detailed tracing */
  debug: (...args) => {
    if (window.__fabricfmt_debug__) {
      console.log('[fabricfmt:debug]', ...args);
    }
  },
};

// Expose for testing (will be accessible via window.__fabricfmt)
if (typeof window !== 'undefined') {
  window.__fabricfmt = { formatCell };
}
...
/**
 * Map detected cell type to fabricfmt language identifier
 */
